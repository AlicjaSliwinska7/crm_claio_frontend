...
export default function TasksSchedule({
  ...
  const viewStart = dFromStr(from) || startOfWeekMon(today);
  ...
  const gotoTodayWeek = () => setRangeMode('ten_tydzien'); // ✅ bez literówki
  ...
  const days = useMemo(
    () => Array.from({ length: visibleDays }, (_, i) => addDays(viewStart, i)),
  ...
  const clearAllFilters = () => {
    ...
    setKindSet(new Set(kindsInBase));
    setHl(null);
  };
  ...
  const prelim = useMemo(() => {
    ...
    const hit = (t) =>
      norm([t.title, t.type, t.status, t.kind, ...(t.tests || []), ...(t.assignees || [])].join('|')).includes(
        norm(q),
      );
  ...
  const rootClass = `tasksschedule${compact ? ' theme-compact' : ''}${flat ? ' theme-flat' : ''}${hl ? ' hl-mode' : ''}`;
    ...
    <div className={rootClass} style={{ ['--ts-left-w']: `${LEFT_W}px` }}>
      ...
      <div className="ts-toolbar ts-toolbar-grid">
        ...
        <div className="group clear-group">
          <button type="button" className="btn-icon" onClick={clearAllFilters} aria-label="Wyczyść filtry" title="Wyczyść filtry">
            ×
          </button>
        </div>
      </div>
              ...
              {[1, 2, 3].map((n) => {
                ...
                return (
                  <button
                    key={n}
                    type="button"
                    className={`ts-chip diff-chip l${n} ${diffSet.has(n) ? 'is-on' : ''} ${hlOn ? 'is-hl' : ''}`}
                    onClick={() => {
                      setDiffSet((prev) => {
              ...
              {[1, 2, 3].map((n) => {
                ...
                return (
                  <button
                    key={n}
                    type="button"
                    className={`ts-chip prio-chip p${n} ${prioSet.has(n) ? 'is-on' : ''} ${hlOn ? 'is-hl' : ''}`}
                    onClick={() => {
                      setPrioSet((prev) => {
                        const s = new Set(prev);
                        if (s.size === 1 && s.has(n)) return s;
        ...
        <div className="ts-headrow">
          <div className="ts-corner" style={{ width: LEFT_W }}>
            <div className="c-month">
              ...
              {days[0] && monthName(days[0]) !== monthName(days[visibleDays - 1])
                ? ` / ${monthName(days[visibleDays - 1])}`
                : ''}
            </div>
              ...
              <div className="ts-row" key={row.emp}>
                ...
                <div className="ts-lane" style={{ width: contentW, height: Math.max(48, row.lanesCount * 48) }}>
                  ...
                  {(leaves || [])
                    ...
                    .map((l, i) => {
                      const s = dFromStr(l.from);
                      const e = dFromStr(l.to);
                      if (!s || !e) return null;
        ...
        {today >= viewStart && today <= viewEnd && (
          <div
            style={{
              ...
              top: 0,
              bottom: 0,
              left: LEFT_W + diffDays(viewStart, today) * dayW,
              width: 1,
      ...
      <div className="ts-nav">
        <div className="controls">
          <button type="button" className="nav-btn pill" onClick={() => shiftWindowBy(-7)} title="Wstecz o tydzień">
            « 7
          </button>
          <button type="button" className="nav-btn round icon" onClick={() => shiftWindowBy(-1)} aria-label="Poprzedni dzień">
            <ChevronLeft size={ICON_SIZE} />
          </button>

          {/* ✅ FIX: wcześniej było onClick={()=>gotoTodayWeek} (nie wywoływało) */}
          <button type="button" className="nav-btn primary" onClick={gotoTodayWeek}>
            Dziś (tydz.)
          </button>

          <button type="button" className="nav-btn round icon" onClick={() => shiftWindowBy(1)} aria-label="Następny dzień">
            <ChevronRight size={ICON_SIZE} />
          </button>
          <button type="button" className="nav-btn pill" onClick={() => shiftWindowBy(7)} title="Naprzód o tydzień">
            7 »
          </button>
        </div>
      </div>
