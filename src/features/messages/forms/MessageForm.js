...
/**
 * Jeden kontekst wiadomości dla całej aplikacji.
 * Jeśli używasz go także w LowerNavBar, pamiętaj by App owijał całość <MessagesProvider>.
 */
...
export function MessagesProvider({
  ...
  const addMessage = useCallback(
    (chatId, { text, sender = loggedInUserId }) => {
      const msg = {
        id: rid(),
        sender,
        text,
        timestamp: nowIso(),
        read: sender === loggedInUserId, // moje wiadomości od razu „przeczytane”
        reactions: {},
      }
  ...
  const markAllReadInConversation = useCallback(
    (chatId) => {
...
// ───────────────────────── re-exports (opcjonalnie) ─────────────────────────
// Jeśli przeniesiesz to do src/app/providers/MessagesProvider.jsx,
// zostaw tutaj plik-wskaźnik (re-export), by nie zmieniać importów w features:
// export { MessagesProvider, useMessages } from '../../../app/providers/MessagesProvider.jsx'
