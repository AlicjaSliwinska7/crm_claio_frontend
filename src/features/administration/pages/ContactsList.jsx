...
// ✅ spójny generator ID (masz go już w projekcie – używasz np. w appointments)
...
export default function ContactsList() {
  ...
  const departmentsCount = useMemo(() => {
    const set = new Set(filteredSorted.map((c) => (c?.department || '').trim()).filter(Boolean))
    return set.size
  }, [filteredSorted])

  const buildingsCount = useMemo(() => {
    const set = new Set(filteredSorted.map((c) => (c?.building || '').trim()).filter(Boolean))
    return set.size
  }, [filteredSorted])
  ...
  const exportCSV = useCsvExport({
    columns: CSV_COLUMNS,
    rows: filteredSorted,
    filename: csvFilename('kontakty'), // ✅ spójnie z resztą list
