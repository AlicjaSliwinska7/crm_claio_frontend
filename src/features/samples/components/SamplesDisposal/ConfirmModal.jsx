import React from 'react'
import Modal from '../../../../shared/modals/modals/Modal'
import '../../../../shared/modals/styles/form-modal.css' // „blue underline” modal

export default function ConfirmModal({ open, date, setDate, onConfirm, onClose, size = 'sm' }) {
	if (!open) return null

	return (
		<Modal title='Potwierdź utylizację próbek' onClose={onClose} size={size}>
			<form
				className='m-form'
				onSubmit={e => {
					e.preventDefault()
					onConfirm()
				}}>
				<div className='m-field'>
					<label className='m-label' htmlFor='dispose-date'>
						Data utylizacji
					</label>
					<input
						id='dispose-date'
						type='date'
						className='m-input'
						value={date}
						onChange={e => setDate(e.target.value)}
					/>
				</div>

				<div className='m-actions--footer' role='toolbar' aria-label='Akcje potwierdzenia utylizacji'>
					<button type='submit' className='m-btn m-btn--primary'>
						Zatwierdź
					</button>
					<button type='button' className='m-btn m-btn--secondary' onClick={onClose}>
						Anuluj
					</button>
				</div>
			</form>
		</Modal>
	)
}
