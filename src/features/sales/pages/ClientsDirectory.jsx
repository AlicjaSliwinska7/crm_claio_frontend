// src/features/sales/pages/ClientsDirectory.jsx
...
export default function ClientsDirectory() {
  ...
  // ✅ kluczowe: stabilna referencja, żeby useListCrud nie wpadał w pętlę setState
  const initialItems = useMemo(() => normalizeOnLoad(initialClients), [])
  ...
  const {
    ...
    initialItems, // ✅ zamiast normalizeOnLoad(initialClients) inline
    idKey: 'id',
    makeId: () => rid('C'),
    validate,
    normalizeOnSave,
  ...
  const {
    ...
    filteredSorted,
    total,
  } = useListQuery(clients, HEADER_COLS, {
  ...
  const handleRowClick = useCallback(
    (id) => navigate(`/sprzedaz/klienci/${encodeURIComponent(id)}`),
    [navigate]
  )

  // ✅ nie nawiguj, jeśli klik pochodzi z elementu interaktywnego
  const isInteractiveTarget = (target) => {
    if (!target) return false
    return Boolean(
      target.closest?.(
        'a,button,input,select,textarea,label,[role="button"],[role="link"],[data-stop-row-click]'
      )
    )
  }
