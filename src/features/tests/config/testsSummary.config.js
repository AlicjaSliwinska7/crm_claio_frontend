...
// ✅ nowa sekcja filtrów
import TestsFilters from '../components/TestsSummary/Filters'
...
export function buildTestsSummaryConfig(ctx) {
  return summaryPage.summaryPage('tests-summary ts-page es-root', [
    // ✅ Filtry główne
    summarySections.filtersSection('tests-filters', TestsFilters, {
      filter: ctx.filter,
      setFilter: ctx.setFilter,
      accrFilter: ctx.accrFilter,
      setAccrFilter: ctx.setAccrFilter,

      rangePreset: ctx.rangePreset,
      setRangePreset: ctx.setRangePreset,
      from: ctx.from,
      setFrom: ctx.setFrom,
      to: ctx.to,
      setTo: ctx.setTo,

      onReset: ctx.onResetFilters,
    }),
